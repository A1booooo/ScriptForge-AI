...
- `apps/web`
  - 提供 T04/T05/T08 前端章节输入工作台
  - 当前能力边界包含章节输入、改编模式选择、提交状态展示、mock 剧本摘要、只读 YAML 预览、轻量 Preview checks、Scene Board 和 Character Bible
...
- 成功后展示结果工作台：
  ...
  - 提示当前 Preview Checks 仍是 lightweight panel，shared validator runtime 尚未接入这个 UI

当前明确不做：

- shared validator runtime 的 UI 接入
- Quality Score
- Rewrite Modes
- 导出
- 历史记录
...
- `react-dom`
  - 将 React 应用挂载到浏览器
- `yaml`
  - 用于 `apps/web` 的只读 YAML 预览 stringify，以及 `packages/shared` 的 T07 parser runtime
- `ajv`
...
当前 README 只描述仓库中已经真实存在的能力。当前边界包括前端章节输入工作台、Mock API 调用、基础请求状态与摘要展示、只读 YAML 预览、轻量 Preview checks、Scene Board、Character Bible、API 内部 LLM client boundary / config / mockable provider client / draft service、`@scriptforge/shared` YAML parser / JSON Schema validator runtime / consistency checks、项目级 CI 以及根 verification scripts；不包含真实 LLM public endpoint、前端主链路接入、shared validator runtime 的完整 UI 接入、Quality Score、Rewrite Modes、导出、历史记录、用户系统、Docker，或任何未完成的后续能力。
