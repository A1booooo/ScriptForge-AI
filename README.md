...
- `apps/web`
  - 提供 T04 前端章节输入工作台
  - 当前能力边界只包含章节输入、改编模式选择、提交状态展示和 mock 剧本摘要
...
当前 README 只描述仓库中已经真实存在的能力。T04 的边界仅包括前端章节输入工作台、Mock API 调用以及基础请求状态与摘要展示，不包含真实 LLM、YAML 解析与校验、Scene Board、Character Bible、Quality Score、Rewrite Modes、导出、历史记录、用户系统、CI 或 Docker。
