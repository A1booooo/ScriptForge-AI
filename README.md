...
- `apps/web`
  ...
  - 当前能力边界包含章节输入、改编模式选择、提交状态展示、mock 剧本摘要、Generated YAML 基线、Edited YAML 编辑、shared validator runtime 校验结果、`.yaml` 导出、轻量 Preview checks、Scene Board 和 Character Bible
...
T04/T05/T08/T09 对应的前端工作台，当前页面能力包括：
...
- 成功后展示结果工作台：
  ...
  - 提示当前 Preview Checks 仍是 lightweight panel；shared validator runtime 已用于 Edited YAML 校验，Scene Board / Character Bible 仍展示 generated draft 视图
...
当前 README 只描述仓库中已经真实存在的能力。当前边界包括前端章节输入工作台、Mock API 调用、基础请求状态与摘要展示、Generated YAML 基线、Edited YAML 编辑、Validation Result、`.yaml` 导出、轻量 Preview checks、Scene Board、Character Bible、API 内部 LLM client boundary / config / mockable provider client / draft service、`@scriptforge/shared` YAML parser / JSON Schema validator runtime / consistency checks、项目级 CI 以及根 verification scripts；不包含真实 LLM public endpoint、前端主链路接入、Quality Score、Rewrite Modes、历史记录、用户系统、Docker，或任何未完成的后续能力。
