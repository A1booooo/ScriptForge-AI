...
当前 README 只描述仓库中已经真实存在的能力。T05 / C01 的边界包括前端章节输入工作台、Mock API 调用、基础请求状态与摘要展示、只读 YAML 预览、轻量 Preview checks、项目级 CI 以及根 verification scripts；不包含真实 LLM、完整 YAML parser、完整 JSON Schema validator runtime、Scene Board、Character Bible、Quality Score、Rewrite Modes、导出、历史记录、用户系统、Docker，或任何未完成的后续能力。
