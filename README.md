...
- T13: `@scriptforge/web` adds clearly marked Demo fixtures and a one-click `Run Demo Sample` path that loads and submits the Demo sample through the existing mock conversion flow.
...
- T12：`@scriptforge/web` Rewrite Suggestions deterministic Demo 建议展示
...
- `apps/web`
  - T13 adds a frontend-local Demo fixture in `apps/web/src/lib/demoFixtures.ts` plus a `Run Demo Sample` entry that reuses `POST /api/conversions/mock`.
...
T13 note:
- `Run Demo Sample` loads a clearly marked Demo sample and immediately submits it through the existing mock conversion flow.
- Demo disclosure appears in both the entry area and the result summary.
- This does not add a real LLM path, a demo endpoint, or any new analysis logic beyond the existing deterministic Demo panels.
...
- 成功后展示结果工作台：
  ...
  - Rewrite Suggestions：
    - 基于 generated draft、T10 Chapter Analyzer 输出与 T11 Adaptation Quality Score 输出做 deterministic improvement suggestions
    - 至少覆盖 dialogue enhancement、pacing adjustment、scene compression 三类 rewrite mode
    - 每条 suggestion 包含 mode、target、reason 与 signal source
    - 明确标注为 deterministic Demo suggestions，不是真实 LLM rewrite，不会自动应用，也不会修改 YAML
  - Scene Board：
...
T13 boundary update:
- Demo fixtures stay local to `apps/web/src/lib/demoFixtures.ts`.
- The demo path reuses `POST /api/conversions/mock` and does not change the API public contract or shared schema.
- The repo still does not include real LLM generation, history, user accounts, database storage, or complex routing.

当前 README 只描述仓库中已经真实存在的能力。当前边界包括前端章节输入工作台、Mock API 调用、基础请求状态与摘要展示、Chapter Analyzer deterministic Demo analysis、Adaptation Quality Score deterministic Demo 评分、Rewrite Suggestions deterministic Demo 建议、Generated YAML 基线、Edited YAML 编辑、Validation Result、`.yaml` 导出、轻量 Preview checks、Scene Board、Character Bible、API 内部 LLM client boundary / config / mockable provider client / draft service、`@scriptforge/shared` YAML parser / JSON Schema validator runtime / consistency checks、项目级 CI 以及根 verification scripts；不包含真实 LLM public endpoint、前端主链路接入、真实 LLM 推理分析、自动改写输出、历史记录、用户系统、Docker，或任何未完成的后续能力。
